...
**Gate 2E.2E.1-R:** F8 executable traveler is `D1`. The previous `c56b2bcc` mismatch (`F_discovery_forward` D2 vs F8 D1 label) is corrected by an F8-only posture override. `F_discovery_forward` itself remains D2 for Gate 2A. Explicit D1 profile QA under AC remains a separate probe.
...
- F8’s Route Lab label and executable fixture are **D1** (Gate 2E.2E.1-R). Tabulated F8 lane scores below were recorded at `c56b2bcc` when the traveler was still D2; they are historical, not a 2E.2E.1-R oracle.
