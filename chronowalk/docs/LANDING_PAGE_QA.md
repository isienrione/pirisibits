...
| Check | How to test | Expected result |
|-------|-------------|-----------------|
| Live price | Look at purchase buttons and value lines | Price matches app config (e.g. **€14.99**), not a random hardcoded value in multiple places. |
| Single fallback | Ask a developer or search code only if needed | Fallback (e.g. €14.99 full bundle) is defined **once**, not copied everywhere. |
| Currency | Product is EUR | Shows **€**, not **$**. |
