...
`VITE_MAPBOX_TOKEN` is empty, the native map can still open (SDK token) and
packaged Rome stop→stop legs still resolve offline; live Mapbox Directions will
not. Set both to the same public `pk.` token for full Simulator builds.
